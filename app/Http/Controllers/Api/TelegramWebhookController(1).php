...
class TelegramWebhookController extends Controller
{
        ...
        try {
            ...
            if (!$company) {
                ...
                return response()->json([
                    'error' => 'Marketplace não permitido ou não cadastrado'
                ], 422);
            }
    ...
    private function sendToTelegram_R00(Product $product, $photoPath = null): bool
    {
        $token  = env('TELEGRAM_BOT_TOKEN');
        ...
        $caption =
            ...
            "⚠️ <i>Promoção sujeita a alteração.</i>";
    
        $filePath = $photoPath && !filter_var($photoPath, FILTER_VALIDATE_URL)
            ? base_path($photoPath)
            : null;
        ...
        Log::info('📤 Enviando produto para o Telegram', [
            'product_id' => $product->id,
            'chat_id' => $chatId
        ]);
        ...
        if ($error) {
            Log::error('❌ Erro ao enviar para Telegram', [
                'error' => $error
            ]);
            return false;
        }
    
        Log::info('✅ Produto enviado ao Telegram com sucesso', [
            'product_id' => $product->id
        ]);
    ...
    private function sendToTelegram(Product $product, $photoPath = null): bool
    {
        ...
        */
        ...
        $isMercadoLivre  = str_contains($affiliateLink, 'mercadolivre.');
    
        $link = ($isAmazon || $isMercadoLivre)
