...
class TelegramWebhookController extends Controller
{
    ...
    {
        // 🔧 Remove emojis usando padrão compatível com PHP
        // Substitui caracteres emoji por espaço (faixa comum de emojis)
        ...
        // Debug log (remova depois que funcionar)
        Log::debug('🔍 ExtractPrice debug', [
            'type' => $type,
            'clean_text' => substr($cleanText, 0, 200)
        ]);
        
        if ($type === 'promo') {
            // Padrões para preço promocional (em ordem de prioridade)
            $patterns = [
                '/Por\s+(?:apenas\s+)?R?\$?\s*([\d.,]+)/i',  // "Por R$ 123,45" ou "Por apenas 123,45"
                '/Por[^\d]*([\d]+[,.]\d{2})/i',              // "Por 123,45"
                '/(?:R\$?\s*)?([\d]+[,.]\d{2})\s*(?:🔥|🚀)/i' // Fallback com emoji
            ];
            
            foreach ($patterns as $pattern) {
                if (preg_match($pattern, $cleanText, $m)) {
                    $price = trim($m[1]);
                    Log::info('✅ Preço promocional extraído', ['price' => $price]);
                    return $price;
                }
            }
        }
        ...
        if ($type === 'original') {
            // Padrões para preço original
            $patterns = [
                '/De\s+(?:R?\$?\s*)?([\d.,]+)/i',            // "De R$ 123,45" ou "De 123,45"
                '/(?:R\$?\s*)?([\d]+[,.]\d{2})\s*(?:~|<s>)/i' // Fallback com tag de riscado
            ];
            
            foreach ($patterns as $pattern) {
                if (preg_match($pattern, $cleanText, $m)) {
                    $price = trim($m[1]);
                    Log::info('✅ Preço original extraído', ['price' => $price]);
                    return $price;
                }
            }
        }
        
        Log::warning('❌ Preço não encontrado', ['type' => $type, 'text' => substr($cleanText, 0, 100)]);
        return null;
    }
    ...
    private function sendToTelegram(Product $product, $photoPath = null): bool
    {
        ...
        */
        ...
        $promoPrice = $product->promo_price
            ...
            /*
            |--------------------------------------------------------------------------
            | 📉 CALCULAR DESCONTO (FÓRMULA)
            |--------------------------------------------------------------------------
            */
            $discountPercent = null;
            if ($product->original_price && $product->promo_price && $product->original_price > 0) {
                $discount = (($product->original_price - $product->promo_price) / $product->original_price) * 100;
        ...
        |--------------------------------------------------------------------------
        */
        $discountBlock = $discountPercent ? "📉 Desconto: <b>{$discountPercent} OFF</b>\n\n" : "";
        ...
        */
        ...
        $caption =
            "🛍 {$product->title}\n\n" .
            $priceBlock .
            $discountBlock .  // ← ✅ AQUI ESTAVA FALTANDO!
            "🛒 Ver oferta da {$companyName} 👇\n" .
            "{$link}\n\n" .
            "⚠️ _Promoção sujeita a alteração de preço e estoque._";
        ...
        |--------------------------------------------------------------------------
        */
        $filePath = $photoPath && !filter_var($photoPath, FILTER_VALIDATE_URL) ? public_path($photoPath) : null;
    ...
    private function downloadImage(string $url): ?string
    {
        ...
        $name = uniqid() . '.' . $ext;
        $dir = public_path('uploads/imgProducts/');
