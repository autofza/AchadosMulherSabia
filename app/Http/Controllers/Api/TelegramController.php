...
class TelegramController
{
    ...
    public function sendProduct(Product $product, ?string $photoPath = null): bool
    {
        ...
        if (!$token || !$chatId) {
            ...
            $this->auditError(
                event: 'telegram.send.not_configured',
                message: 'Token ou Chat ID não configurados',
                model: $product
            );
        ...
        if (
            ...
            $this->auditError(
                event: 'telegram.send.invalid_image',
                message: 'Imagem ausente ou padrão',
                model: $product
            );
        ...
        if (!file_exists($filePath)) {
            ...
            $this->auditError(
                event: 'telegram.send.image_not_found',
        ...
        if (!$response || empty($response['ok'])) {
            ...
            $this->auditError(
                ...
                message: 'Erro retornado pela API do Telegram',
                model: $product
            );
