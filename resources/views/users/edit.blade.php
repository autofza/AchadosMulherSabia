    ...
    <form action="{{ route('users.update', ['user' => $user->id]) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
            <div class="mb-2">
                ...
                <input type="text" name="name" id="name" class="form-input" placeholder="Nome completo do usuário" value="{{ old('name', $user->name) }}" required>
            </div>

            <div class="mb-2">
                <label for="email" class="form-label">E-mail</label>
                <input type="text" name="email" id="email" class="form-input" placeholder="Melhor e-mail do usuário" value="{{ old('email', $user->email) }}" required>
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="checkbox" name="roles[]" id="role_{{ Str::slug($role) }}" value="{{ $role }}" {{ in_array($role, old('roles', $userRoles)) ? 'checked' : '' }}>
